    ...
    private void displayRequests(){
        if(requestBeans.isEmpty()){
            ...
            for(int i=0;i<requestBeans.size();i++){
                RequestBean request = requestBeans.get(i);
                Printer.println((i+1) + ". Richiesta da: " + request.getPatientBean().getFullName() + "| "+ request.getPatientBean().getCity()+"|  " + request.getDate()+ "| " + request.getPatientBean().getModality());
            }
        }
    }
